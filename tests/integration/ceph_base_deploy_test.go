...
func StartTestCluster(t func() *testing.T, minimalMatrixK8sVersion, namespace, storeType string, useHelm, useDevices, usePVC bool, storageClassName string, mons,
	...
	op := &TestCluster{i, kh, nil, t, namespace, storeType, storageClassName, useDevices, usePVC, mons, rbdMirrorWorkers}
...
func (op *TestCluster) Setup() {
	isRookInstalled, err := op.installer.InstallRookOnK8sWithHostPathAndDevicesOrPVC(op.namespace, op.storeType,
		op.useDevices, op.usePVC, op.storageClassName, cephv1.MonSpec{Count: op.mons, AllowMultiplePerNode: true}, false /* startWithAllNodes */, true, op.rbdMirrorWorkers)
